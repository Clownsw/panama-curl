...
public interface TIMECALLBACK {
    ...
    static MemorySegment allocate(TIMECALLBACK fi, MemorySession session) {
        return RuntimeHelper.upcallStub(TIMECALLBACK.class, fi, constants$687.TIMECALLBACK$FUNC, session);
    }
    static TIMECALLBACK ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _uTimerID, int _uMsg, long _dwUser, long _dw1, long _dw2) -> {
            try {
                constants$687.TIMECALLBACK$MH.invokeExact((Addressable)symbol, _uTimerID, _uMsg, _dwUser, _dw1, _dw2);
            } catch (Throwable ex$) {
