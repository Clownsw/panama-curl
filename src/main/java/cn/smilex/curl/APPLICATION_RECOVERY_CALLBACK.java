...
public interface APPLICATION_RECOVERY_CALLBACK {
    ...
    static MemorySegment allocate(APPLICATION_RECOVERY_CALLBACK fi, MemorySession session) {
        return RuntimeHelper.upcallStub(APPLICATION_RECOVERY_CALLBACK.class, fi, constants$398.APPLICATION_RECOVERY_CALLBACK$FUNC, session);
    }
    static APPLICATION_RECOVERY_CALLBACK ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (java.lang.foreign.MemoryAddress _pvParameter) -> {
            try {
                return (int)constants$398.APPLICATION_RECOVERY_CALLBACK$MH.invokeExact((Addressable)symbol, (java.lang.foreign.Addressable)_pvParameter);
            } catch (Throwable ex$) {
