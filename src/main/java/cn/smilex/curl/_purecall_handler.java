...
public interface _purecall_handler {
    ...
    static MemorySegment allocate(_purecall_handler fi, MemorySession session) {
        return RuntimeHelper.upcallStub(_purecall_handler.class, fi, constants$991._purecall_handler$FUNC, session);
    }
    static _purecall_handler ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$991._purecall_handler$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
