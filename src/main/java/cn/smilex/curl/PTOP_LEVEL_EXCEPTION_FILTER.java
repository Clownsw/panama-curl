...
public interface PTOP_LEVEL_EXCEPTION_FILTER {
    ...
    static MemorySegment allocate(PTOP_LEVEL_EXCEPTION_FILTER fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PTOP_LEVEL_EXCEPTION_FILTER.class, fi, constants$217.PTOP_LEVEL_EXCEPTION_FILTER$FUNC, session);
    }
    static PTOP_LEVEL_EXCEPTION_FILTER ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (java.lang.foreign.MemoryAddress _ExceptionInfo) -> {
            try {
                return (int)constants$218.PTOP_LEVEL_EXCEPTION_FILTER$MH.invokeExact((Addressable)symbol, (java.lang.foreign.Addressable)_ExceptionInfo);
            } catch (Throwable ex$) {
