...
public interface PHANDLER_ROUTINE {
    ...
    static MemorySegment allocate(PHANDLER_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PHANDLER_ROUTINE.class, fi, constants$628.PHANDLER_ROUTINE$FUNC, session);
    }
    static PHANDLER_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _CtrlType) -> {
            try {
                return (int)constants$629.PHANDLER_ROUTINE$MH.invokeExact((Addressable)symbol, _CtrlType);
            } catch (Throwable ex$) {
