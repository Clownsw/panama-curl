...
public interface PSECURE_MEMORY_CACHE_CALLBACK {
    ...
    static MemorySegment allocate(PSECURE_MEMORY_CACHE_CALLBACK fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PSECURE_MEMORY_CACHE_CALLBACK.class, fi, constants$185.PSECURE_MEMORY_CACHE_CALLBACK$FUNC, session);
    }
    static PSECURE_MEMORY_CACHE_CALLBACK ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (java.lang.foreign.MemoryAddress _Addr, long _Range) -> {
            try {
                return (byte)constants$185.PSECURE_MEMORY_CACHE_CALLBACK$MH.invokeExact((Addressable)symbol, (java.lang.foreign.Addressable)_Addr, _Range);
            } catch (Throwable ex$) {
