...
public interface PTHREAD_START_ROUTINE {
    ...
    static MemorySegment allocate(PTHREAD_START_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PTHREAD_START_ROUTINE.class, fi, constants$191.PTHREAD_START_ROUTINE$FUNC, session);
    }
    static PTHREAD_START_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (java.lang.foreign.MemoryAddress _lpThreadParameter) -> {
            try {
                return (int)constants$191.PTHREAD_START_ROUTINE$MH.invokeExact((Addressable)symbol, (java.lang.foreign.Addressable)_lpThreadParameter);
            } catch (Throwable ex$) {
