...
public interface BAD_MEMORY_CALLBACK_ROUTINE {
    ...
    static MemorySegment allocate(BAD_MEMORY_CALLBACK_ROUTINE fi, MemorySession session) {
        return RuntimeHelper.upcallStub(BAD_MEMORY_CALLBACK_ROUTINE.class, fi, constants$269.BAD_MEMORY_CALLBACK_ROUTINE$FUNC, session);
    }
    static BAD_MEMORY_CALLBACK_ROUTINE ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$269.BAD_MEMORY_CALLBACK_ROUTINE$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
